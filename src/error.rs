...
pub enum ArgError {
    ...
    // This error is raised if the user hasn't specified the -o or --output argument but only -N or --no-display without any value after
    #[error("no output file specified without -o or --output argument\nUsage: worgenx_cli <command> [options]\nTry 'worgenx_cli --help' for more information.")]
    NoStandardOutput,
}
